...
- 🚀 **[Quick Start & Integration](./quick-start.md)** — Step-by-step installation, running locally, and middleware integration examples.
- 🏗️ **[System Architecture](./architecture.md)** — In-depth breakdown of Spring Boot, local caching, Redis Lua scripts, and request execution flows.
- ⚙️ **[Rate Limiting Algorithms](./algorithms.md)** — Mathematical and implementation details for Token Bucket and Sliding Window algorithms.
- 🔑 **[Understanding Identifiers](./identifiers.md)** — How to isolate limits using User IDs, API Keys, IP Addresses, and multi-tenant keys.
- 📡 **[API Reference](./api-reference.md)** — Complete OpenAPI specification for `/check` and `/admin/client/*` endpoints.
- 🛠️ **[Configuration Reference](./configuration.md)** — Environment variables (`REDIS_URL`, `PORT`) and Client entity parameter schemas.
- ⚠️ **[Error Handling](./error-handling.md)** — Standard HTTP 429 response handling and `Retry-After` header calculations.
- 📊 **[Performance Optimization Journey](./performance.md)** — Engineering case study, Gatling benchmark analysis, and production tuning recommendations.
